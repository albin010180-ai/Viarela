(function () {
  ...
  const t = lang === 'tr' ? {
    ...
    rate: 'Rate: 1 XMR ≈ {r} EUR · covers {e} EUR on your invoice',
  ...
  function active(dataset) {
    return root.querySelector('[data-' + dataset + '].is-on');
  }
  ...
  els.create.addEventListener('click', async () => {
    ...
    const packBtn = active('pack');
    const stageBtn = active('stage');
    const packageId = packBtn ? packBtn.dataset.pack : 'horizon';
    const stage = stageBtn ? stageBtn.dataset.stage : 'retainer';
    ...
    try {
      const resp = await fetch('/api/xmr/invoice/', {
        ...
        body: JSON.stringify({ package_id: packageId, stage })
      });
  ...
  function startExpiry(iso) {
    ...
    expiryTick(new Date(iso).getTime() - Date.now(), true);
  }

  function expiryTick(left, immediate) {
    if (els.expires && immediate) els.expires.textContent = t.expiresIn.replace('{m}', String(Math.ceil(left / 60000)));
  }
